...
MARGINS:QMargins = QMargins(2, 2, 2, 2)
SPACING:int = 5
...
class details(QWidget):
    def __init__(self, dictionary:LanguagePack, row_to_display:row, parent:main) -> None:
        ...
        'Room', 'Additional Pass Info', 'Additional Info']:
            ...
            small_box.layout().setContentsMargins(MARGINS)
            small_box.layout().setSpacing(SPACING)
...
class main(QWidget):
    def __init__(self, dictionary:LanguagePack, database:list[row]) -> None:
        ...
        for w in [self.filter_label, self.course_name_label, self.suggested_learning_stage_label, self.teacher_name_label,
        self.place_limit_label, self.course_type_label, self.test_type_label, self.hours_winter_label, self.hours_summer_label,
        self.ects_winter_label, self.ects_summer_label, self.ects_combined_label, self.faculty_label, self.faculty_name_label,
        self.weekday_label, self.start_hour_label, self.end_hour_label, self.room_label]:
            w.setFixedWidth(200)
            w.setWordWrap(True)
        ...
        menu = self.menu_bar.addMenu('Plik')
        self.load_db_action:QAction = QAction("Załaduj bazę")
    ...
    def load_database(self) -> None:
        ...
        file_name, _ = QFileDialog.getOpenFileName(self, "Załaduj plik", "", "CSV Files (*.csv);", options=options)

        if file_name:
            QMessageBox.information(self, "Sukces", f"Załadowano bazę {file_name}")
            self.database = read_all_rows(get_resource_path(file_name))
            self.clear_filters_button.click()
        else:
            QMessageBox.information(self, "Bład", f"Ładowanie bazy nie powiodło się")    
    ...
    def create_header_row(self) -> QGroupBox:
        ...
        course_name_label.setFixedWidth(320)
        teacher_label.setFixedWidth(250)
        weekday_label.setFixedWidth(100)
        start_hour_label.setFixedWidth(80)
        end_hour_label.setFixedWidth(80)
        room_label.setFixedWidth(80)
        ects_combined_label.setFixedWidth(200)
    ...
    def create_course_row(self, row_to_display:row) -> QGroupBox:
        ...
        end_hour_label.setFixedWidth(80)
        room_label.setFixedWidth(80)
        ects_combined_label.setFixedWidth(80)
        ...
        main_box.layout().addWidget(details_button)
...
class app:
    def __init__(self, dictionary:LanguagePack, path:str) -> None:
        ...
        with open(get_resource_path("style\\style.css"), "r") as f:
